...
label Worldmap:
    if Current_Time == 'Вечер':
        scene Crossroads_E onlayer backdrop
    elif Current_Time == 'Ночь':
...
label Misplaced:
        if "caught" in R_RecentActions:
                ...
                "Вы немедленно возвращаетесь в свои комнаты."
...
# Player Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    menu:
        "Вы в своей комнате. Чем бы Вы хотели заняться?"
        "Chat":
                    call Chat
        ...
        "Спать" if Current_Time == "Ночь":
                    call Round10
                    call Girls_Location
                    call EventCalls
        "Ждать" if Current_Time != "Ночь":
        ...
        "Go to Kitty's Room" if TravelMode and "met" in K_History:
                    jump Kitty_Room_Entry
        "Пойти в Душ" if TravelMode:
                    jump Shower_Room_Entry

        "Уйти" if not TravelMode:
                    call Worldmap
        "Уйти [[Go to Campus Square]" if TravelMode:
                    jump Campus_Entry
...
label Rogue_Room_Entry:
    ...
    if "Rogue" in Party:
                    if Current_Time == "Ночь" or (Current_Time == "Вечер" and Round <= 10):
...
label Campus:
    ...
    call Checkout(1)
    call GirlsAngry
    if Current_Time == "Вечер" and "yesdate" in P_DailyActions:
...
# Class Room Menu Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    menu:
        ...
        "There are no classes right now (locked)" if Weekday >= 5 or Current_Time == "Вечер" or Current_Time == "Night":
                pass
        ...
        "Lock the door" if "lockedclass" not in P_RecentActions:
                    if Current_Time == "Вечер" or Weekday >=5:
                            "You lock the door"
...
label Kitty_Room_Entry:
    ...
    if "Kitty" in Party:
                    if Current_Time == "Night" or (Current_Time == "Вечер" and Round <= 10):
...
label Study_Room_Explore:
    ...
    $ D20 = renpy.random.randint(1, 20)
    menu:
        "Где бы вы хотели посмотреть?"
        "Книжная полка":
            if D20 >= 5 + Cnt:
                    $ Line = "Книга"
            else:
                    "Когда вы осматриваете книжную полку, вы случайно сбиваете одну из книг."
                    "Она падает на пол, и что то маленькое блестит на столе."
        "Левый ящик стола":
            if K_Loc != bg_current:
                    "Кажется, вы не можете открыть его, было бы неплохо, если бы кто-то открыл замок изнутри."
            elif D20 >= 10 + Cnt:
                    $ Line = "Лево"
            else:
                    "Когда вы открываете ящик, он громко скрипит."
    ...
    if not Line:
            ...
            "Кажется, здесь больше нет ничего интересного, может позже?"
    $ Cnt += 3
    jump Study_Room_Explore
